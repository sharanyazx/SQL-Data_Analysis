...
-- we can use subquery but cte increase readability
WITH CTE_Example AS 						-- using WITH we define CTE
(
SELECT gender, SUM(salary) sum , MIN(salary) min , MAX(salary) max , COUNT(salary) count, AVG(salary) avg
...
-- this wont work because its not same syntax
SELECT *
FROM CTE_Example;
...
SELECT gender, ROUND(AVG(`SUM(salary)`/`COUNT(salary)`),2)   roundoff
FROM CTE_Example
GROUP BY gender;
